...
static void pgd_annot_view_set_annot_ink(GtkWidget *table, PopplerAnnotInk *ink_annot, gint *row)
{
    ...
    ink_list = poppler_annot_ink_get_ink_list(ink_annot);

    // Convert the ink list to a string representation
    ink_list_str = g_string_new(NULL);
    GArray **data = (GArray **)ink_list->data;
    for (i = 0; i < ink_list->len; i++) {
        GArray *d = data[i];
        PopplerPoint *di = (PopplerPoint *)(d->data);
        for (j = 0; j < d->len; j++) {
            g_string_append_printf(ink_list_str, "(%.2f, %.2f) ", di[j].x, di[j].y);
        }
        g_string_append(ink_list_str, "\n");
    }

    g_array_unref(ink_list);
...
static void pgd_annots_add_annot(PgdAnnotsDemo *demo)
{
    ...
    case POPPLER_ANNOT_INK: {
        ...
        ink_list = g_array_new(FALSE, FALSE, sizeof(GArray *));
        g_array_set_clear_func(ink_list, (GDestroyNotify)clear_garray);

        ink_path = g_array_new(FALSE, FALSE, sizeof(PopplerPoint));
        g_array_append_val(ink_path, i);
        i.x = width;
        i.y = height;
        g_array_append_val(ink_path, i);
        g_array_append_val(ink_list, ink_path);
        ink_path = g_array_new(FALSE, FALSE, sizeof(PopplerPoint));
        i.y = 0;
        g_array_append_val(ink_path, i);
        i.x = 0;
        ...
        poppler_annot_ink_set_ink_list(POPPLER_ANNOT_INK(annot), ink_list);
        g_array_unref(ink_list);
    } break;
    case POPPLER_ANNOT_STAMP: {
