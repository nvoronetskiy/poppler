...
bool Splash::pathAllOutside(SplashPath *path)
{
    ...
    auto calcLL = [](_SplashPoint a, _SplashPoint b) -> _SplashPoint { //
        return { std::min(a.x, b.x), std::min(a.y, b.y) };
    };
    auto calcUR = [](_SplashPoint a, _SplashPoint b) -> _SplashPoint { //
    ...
    auto ll = calcLL({ x, y }, { x2, y2 });
    auto ur = calcUR({ x, y }, { x2, y2 });
